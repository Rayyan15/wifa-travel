...
<head>
    ...
    <title>@yield('title', 'Portal Agen') — Wifa Travel</title>
...
<body class="bg-slate-50 font-sans antialiased flex h-screen overflow-hidden">
    ...
    {{-- Sidebar --}}
    <aside class="w-60 bg-gradient-to-b from-[#0F3460] to-[#16213E] text-white flex flex-col shadow-xl z-20">
        {{-- Logo --}}
        ...
        <nav class="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
            @php
                $menus = [
                    ['route' => 'agent.dashboard',    'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', 'label' => 'Dashboard'],
                    ['route' => 'agent.leads.index',  'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', 'label' => 'Leads Saya'],
                    ['route' => 'agent.packages.index','icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4', 'label' => 'Daftar Paket'],
                ];
            @endphp
            @foreach($menus as $menu)
                @php $isActive = request()->routeIs($menu['route'] . '*'); @endphp
                <a href="{{ route($menu['route']) }}" class="flex items-center px-3 py-2.5 rounded-lg font-medium text-sm transition duration-200 {{ $isActive ? 'bg-wifa-gold text-[#0F3460]' : 'text-white/70 hover:bg-white/10 hover:text-white' }}">
        ...
        <div class="p-3 border-t border-white/10">
            <div class="flex items-center gap-3 px-3 py-2 mb-2 rounded-lg bg-white/5">
                <div class="w-8 h-8 rounded-full bg-wifa-gold flex items-center justify-center text-[#0F3460] font-bold text-sm">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-6 z-10 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-400">Portal Agen — Wifa Tour & Travel</div>
            </div>
            <div class="flex items-center gap-3">
                <span class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full border border-blue-100">AGEN</span>
            </div>
        </header>
